...
try:
    ...
    def plot_confusion(y_true, y_prob, threshold=0.5, outdir="plots", title="Confusion Matrix"):
        outdir=_ensure_dir(outdir); import matplotlib.pyplot as plt
...
# Sampler & threshold controls
POS_TARGET = 0.25           # target malicious fraction sampled during training
EARLY_STOP_PATIENCE = 2     # epochs w/o val F1+ improvement
THRESHOLD_METRIC = "f1"     # "f1", "f05" (recall-leaning), or "f2" (more recall)
...
# ---- WeightedRandomSampler targeting POS_TARGET ----
...
criterion = nn.CrossEntropyLoss()                     # sampler balances classes → no class weights here
...
# ---- Training Loop with early stopping on val F1 (malicious) ----
...
for epoch in range(EPOCHS):
    ...
    # evaluate @ default 0.5 (we’ll tune threshold next)
    ...
    # early stopping on *best thresholded F1*
...
# ---- Tune threshold on validation, then test with it ----
...
# === Save artifacts for inference (model + config) ===
...
# Also keep a simple text copy of the threshold (optional)
...
# === Confusion counts (TN, FP, FN, TP) on test ===
